...
class Qwen2VLAForConditionalGeneration(Qwen2VLForConditionalGeneration):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
    ...
    def get_rope_index(
        ...
        #TODO: test this method with 1 audio and 1 image on a VLA model instance
        """
        Calculate the 3D rope index based on image and video's temporal, height and width in LLM.
        ...
        if input_ids is not None and (image_grid_thw is not None or video_grid_thw is not None):
            ...
            audio_index, image_index, video_index = 0, 0, 0
            for i, input_ids in enumerate(total_input_ids):
                input_ids = input_ids[attention_mask[i] == 1]
                audio_nums, image_nums, video_nums = 0, 0, 0
                vision_start_indices = torch.argwhere(input_ids == vision_start_token_id).squeeze(1)
                vision_tokens = input_ids[vision_start_indices + 1]
                image_nums = (vision_tokens == image_token_id).sum()
                video_nums = (vision_tokens == video_token_id).sum()
                audio_nums = (input_ids == audio_start_token_id).sum()
                input_tokens = input_ids.tolist()
                llm_pos_ids_list: list = []
                st = 0
                remain_audios, remain_images, remain_videos = audio_nums, image_nums, video_nums
                for _ in range(image_nums + video_nums + audio_nums):
                    ...
                    if ed_image < ed_video and ed_image < ed_audio:
                        ...
                        t, h, w = (
                            audio_grid_thw[audio_index][0],
                            audio_grid_thw[audio_index][1],
                            audio_grid_thw[audio_index][2],
                        )
                        audio_index += 1
                        remain_audios -= 1
                        ed = ed_audio
                    ...
                    if (ed_image < ed_video and ed_image < ed_audio) or (
                        ed_video < ed_image and ed_video < ed_audio):
                        llm_grid_t, llm_grid_h, llm_grid_w = (
                            t.item(),
                            h.item() // spatial_merge_size,
                            w.item() // spatial_merge_size,
                        ...
                        # add position ids for audio
                        audio_len = t.item() * h.item() * w.item()
                        llm_pos_ids_list.append(torch.arange(audio_len).view(1, -1).expand(3, -1) + text_len + st_idx)
                        st = ed + audio_len

                #add position ids for any remaning text after all the vision and audio content
                if st < len(input_tokens):
                    st_idx = llm_pos_ids_list[-1].max() + 1 if len(llm_pos_ids_list) > 0 else 0
                    text_len = len(input_tokens) - st
                    llm_pos_ids_list.append(torch.arange(text_len).view(1, -1).expand(3, -1) + st_idx)
...
__all__ = ["Qwen2VLForConditionalGeneration", "Qwen2VLAForConditionalGeneration", "Qwen2VLModel", "Qwen2VLPreTrainedModel"]
